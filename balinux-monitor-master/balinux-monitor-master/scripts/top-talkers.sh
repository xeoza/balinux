...
do_commands() {
    ...
    if [ $filecount -gt 3 ]; then
        rm `ls -t $out_file_dir -d -1 $out_file_dir/{*,.*} | grep "$daemon_name""_output_" | tail -n +4`
    fi

    sudo timeout 60 tcpdump -nvvv -i any <&- | perl $me_DIR/top-talkers.pl --file "$out_file_dir/$daemon_name""_output_" & perl_pid=$!; wait
}
