...
do_commands() {
    ...
    ss --listening --numeric --tcp --udp | perl $me_DIR/netconn.pl --file "$out_file_dir/$out_file""_ss" --ss
    netstat -utn | perl $me_DIR/netconn.pl --file "$out_file_dir/$out_file""_netstat" --netstat
    if [ $filecount -gt 6 ]; then
        rm `ls -t $out_file_dir -d -1 $out_file_dir/{*,.*} | grep "$daemon_name""_output_" | tail -n +7`
    fi
